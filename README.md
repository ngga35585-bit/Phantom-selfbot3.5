...
Meniu comenzi
,askai <întrebare>
Ghid local Phantom: explică utilizarea, prefixele, audio, voice și depanarea
fără cheie API externă.

Prefix configurat pe token
În `config.json`, tokenul 1 (index `0`) are prefixul `@`, tokenul 2 are `!`,
iar tokenul 3 are `.`. Comenzile voice acceptă atât prefixul configurat pentru
token, cât și prefixul implicit `,` (de exemplu `@help` sau `,help`).

Dacă o comandă nu răspunde, verifică logul procesului: erorile din handler și
erorile de trimitere sunt acum afișate explicit cu eticheta botului.
...
`build.sh` instalează FFmpeg dacă lipsește și reîncearcă automat descărcarea
dependințelor în cazul unor erori temporare de rețea de la PyPI.
Licență
